...
plan: 04-08
phase: 4
title: register-mcp-tools.sh update (+ deck) + verify-phase4.sh + UAT
wave: 4
depends_on:
  - 04-01
  - 04-02
...
# Plan 04-08 — register-mcp-tools.sh update + verify-phase4.sh + UAT

## Goal

Mettre à jour le script de registration MCP pour enregistrer `mcp-deck`, et créer `verify-phase4.sh` avec 8 tests E2E qui valident chaque livrable de Phase 4 — seuil de succès : 6/8 tests verts.

## Context

**Dépendance** : ce plan est Wave 4 — tous les autres plans Phase 4 doivent être exécutés d'abord.

**Pattern référence** : `infrastructure/scripts/verify-phase3.sh` et `register-mcp-tools.sh` existants. Dupliquer le pattern pass/fail/skip + bridge JWT generation + fallback VM host.

**Tests à couvrir** :
1. Container health Phase 4 (mcp-deck Up)
2. POST /v1/messages upsert silencieux (plan 04-01)
3. mcp-gateway aggregate endpoint actif sur port 8081 (plan 04-02)
4. GET /tools via aggregate retourne les tools (plan 04-02 + 04-08)
5. LibreChat config mcpServers (plan 04-03) — smoke test config présente
6. agent-runtime MCP tools discoverable (plan 04-04)
7. drive-sync webhook endpoint actif (plan 04-05)
8. Multi-folder Drive mapping (plan 04-06) — GET /admin/drive-mapping retourne liste

## Tasks

### T-04-08-01 — Mettre à jour `infrastructure/scripts/register-mcp-tools.sh`

**File**: `infrastructure/scripts/register-mcp-tools.sh`
**Action**: modify

**Changes**:

**1.** Mettre à jour le compteur `TOTAL_TOOLS=3` → `TOTAL_TOOLS=4`.

**2.** Dans la section "Register the 3 Phase 3 MCP tools", changer le header :
```bash
echo "--- Registering Phase 3+4 MCP tools ---"
```

**3.** Après les 3 appels `register_tool` existants, ajouter :
```bash
register_tool \
  "deck" \
  "http://mcp-deck:8103" \
...
**4.** Mettre à jour la vérification finale : seuil de succès `≥ 4` tools :
```bash
elif [ "${TOOLS_RESP}" -ge 4 ] 2>/dev/null; then
  pass "${LABEL}" "${TOOLS_RESP} tool(s) registered"
elif [ "${TOOLS_RESP}" -ge 3 ] 2>/dev/null; then
  # 3 tools = Phase 3 OK but deck missing
  fail "${LABEL}" "${TOOLS_RESP} tool(s) registered — deck not registered yet? Run after mcp-deck is up"
```

**5.** Mettre à jour le résumé final :
```bash
TOTAL_TOOLS=4
```
...
### T-04-08-02 — Créer `infrastructure/scripts/verify-phase4.sh`

**File**: `infrastructure/scripts/verify-phase4.sh`
**Action**: create
...
# ---------------------------------------------------------------------------
# TEST 5 — LibreChat config mcpServers présente dans librechat.yaml
# ---------------------------------------------------------------------------
...
SCHEMA_CHECK=$($COMPOSE exec -T postgres \
  python3 -c "
import subprocess, sys
result = subprocess.run(
    ['psql', '-U', '${POSTGRES_USER:-xbrain}', '-d', '${POSTGRES_DB:-xbrain}',
     '-c', \"SELECT column_name FROM information_schema.columns WHERE table_name='team_drive_mappings' AND column_name='project_scope'\"],
    capture_output=True, text=True
)
print('found' if 'project_scope' in result.stdout else 'missing')
" 2>/dev/null) || SCHEMA_CHECK=$(${COMPOSE} exec -T postgres psql -U "${POSTGRES_USER:-xbrain}" -d "${POSTGRES_DB:-xbrain}" -tAc "SELECT column_name FROM information_schema.columns WHERE table_name='team_drive_mappings' AND column_name='project_scope'" 2>/dev/null | tr -d '[:space:]' | head -1)
...
- [ ] `bash infrastructure/scripts/register-mcp-tools.sh` enregistre 4 tools (scraper, drive-read, calendar, deck) et retourne exit 0
- [ ] `bash infrastructure/scripts/verify-phase4.sh` retourne exit 0 avec ≥ 6/8 tests en PASS
- [ ] Test 1 (mcp-deck container) : PASS
- [ ] Test 2 (upsert silencieux) : PASS
- [ ] Test 3 (aggregate port 8081) : PASS
- [ ] Test 4 (4 tools enregistrés) : PASS

## Threat Model

Aucune nouvelle surface d'attaque dans ce plan — les scripts s'exécutent sur la VM en local (pas d'accès réseau externe). Les bridge JWT générés expirent en 5min. Disposition : **accept** — scripts de maintenance locaux uniquement.
