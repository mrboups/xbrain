...
| A3 | `GITHUB_ORG` env on VM = `dejavudev` (default in config.py is now empty — operator must set the value explicitly) | Runtime State Inventory | **Medium** — if the VM env is set to something else, the org-membership check will fail. The plan must include a step to verify the actual VM `.env` value before deploy. |
