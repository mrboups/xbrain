...
- **Auto-index personal (User-install) repos on sign-in (Task #149)** — the repo-catalog auto-triggers (sign-in/team-create) cover ORG installs only: `index_team_catalog`→`find_installation_for_org` calls `GET /orgs/{org}/installation` (404 for a User account). mrboups's 60 personal repos were backfilled MANUALLY (by installation_id) and stay fresh via the `repository` webhook, but are NOT re-indexed on sign-in. Add a User-install fallback (`GET /users/{login}/installation`) to the catalog index path + index the signing-in user's personal repos on sign-in. Ties into Task #147 (which team gets a user's personal repos).
