...
| 260601-uom-searxng-web-search-scraper-auto-fetch | 2026-06-01 | 6209183 | Partial | Make LibreChat read links like claude.ai. ✅ DONE: (1) scraper auto-fetch nudge — promptPrefix on all 5 xbrain modelSpecs → model calls our `scraper` tool on any URL (uses mcp-scraper, no dep; live). (2) SearXNG container (settings.yml JSON-format-on + limiter-off, compose service) — healthy, JSON API verified (9 results). ⛔ BACKED OUT: the LibreChat `webSearch` block crash-looped LibreChat (~2min outage, recovered to RestartCount=0) — TWO blockers: v0.8.5 Zod schema rejects `rerankerType: "none"` (only jina/cohere), AND no valid Firecrawl key (the only fc- on the system is 25 chars + 401s; searched env/settings/@security/plugins). Backout commit 6209183. SearXNG left running idle, ready. TO FINISH (needs user): valid Firecrawl/Tavily key → re-add webSearch block (omit rerankerType or use jina) → redeploy. |
