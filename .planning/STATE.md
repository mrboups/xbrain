...
Last activity: 2026-06-01 — Quick task 260531-x8i: fixed the LibreChat brain differentiator. librechat-bridge resolve_team_scope was a Phase-1 stub always returning team_scope=default, so LibreChat recall+ingest used the wrong brain (real facts live under the user's team slug, e.g. aibrussels). Implemented per-user resolution (new memory-api GET /v1/internal/resolve-team-scope + bridge TTL cache + fail-soft). Live-verified: the GitHub-auth user → aibrussels, enricher recall returns the AI Brussels fact. Also re-seeded mcp_tool_registry (wiped by clean-start → scraper/calendar/etc were 404ing). Deployed surgically to VM (rebuilt memory-api + librechat-bridge).
