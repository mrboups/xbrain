...
- [ ] **DOCBODY-01** (Phase 24): An uploaded document's text body (PDF/DOCX/Markdown/plain text) is extracted, chunked, and embedded via the existing local keyless embedder so it is semantically retrievable from the brain — not just the caption — with the full 7-field tagging inherited from the upload and linked to the MinIO object; size/type guards + a no-text-layer flag; fail-soft (never breaks the upload).
