...
- [x] **CATCHUP-01** (Phase 23): A member opening a busy team chat can OPT IN to a brain-grounded "catch me up" summary of what happened since their last visit — a per-member last_read_at cursor + a since-window query + the existing streaming agent; never auto-run, threshold-gated, ephemeral, rate-limited.
- [x] **DOCBODY-01** (Phase 24): An uploaded document's text body (PDF/DOCX/Markdown/plain text) is extracted, chunked, and embedded via the existing local keyless embedder so it is semantically retrievable from the brain — not just the caption — with the full 7-field tagging inherited from the upload and linked to the MinIO object; size/type guards + a no-text-layer flag; fail-soft (never breaks the upload).
- [x] **JOINCODE-01** (Phase 25): A team admin can mint a shareable invite code (bearer secret); any authenticated user who submits it joins that team's chat. The code is stored hashed (sha256) and returned once; it is revocable, expiring, and max-uses-limited, with every guard enforced at redemption and team_scope-bound (a code for team A can only add to team A). Mint/revoke/list are team-admin-gated; join is rate-limited and idempotent.
- [x] **BOARD-01** (Phase 26a): A team opens a live collaborative Excalidraw board from the chat — two members drawing at once converge in real time, the board survives a reload (Postgres-persisted Y.Doc), and the team-scope boundary is enforced in Hocuspocus's onAuthenticate (a token for team A's board is rejected against team B's document). Delivered as a separate Vite+React SPA built in Docker + a Hocuspocus service in its own container, both OPT-IN profile services that keep the OSS-light 10-service core green.
...
| PKG-02 | Phase 20 | Complete (live-backend UAT residual) |
