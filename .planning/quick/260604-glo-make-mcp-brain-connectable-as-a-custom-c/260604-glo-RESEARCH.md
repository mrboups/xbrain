...
The user pastes the MCP endpoint URL into the connector UI, e.g. `https://mcp.grooveos.app/mcp`. The `/mcp` path is what FastMCP mounts on by default (confirmed by community reports; users paste `https://<host>/mcp`, not just the base URL). [VERIFIED: zenn.dev article, crumrine/fastmcp-personal-auth]
...
2. Claude.ai sends an unauthenticated MCP request → expects `HTTP 401` with `WWW-Authenticate: Bearer resource_metadata="https://mcp.grooveos.app/mcp/.well-known/oauth-protected-resource"`.
3. Claude.ai GETs the protected resource metadata document.
4. Parses `authorization_servers[0]` → constructs AS metadata URL → GETs `https://api.grooveos.app/.well-known/oauth-authorization-server`.
5. Registers as a client via DCR (`POST /register`) unless CIMD is configured.
6. Generates PKCE `code_verifier` + `code_challenge` (S256), includes `resource=https://mcp.grooveos.app/mcp` in the auth request.
...
```json
{
  "resource": "https://mcp.grooveos.app/mcp",
  "authorization_servers": ["https://api.grooveos.app"],
...
WWW-Authenticate: Bearer resource_metadata="https://mcp.grooveos.app/mcp/.well-known/oauth-protected-resource"
```

Must validate that incoming access tokens were **issued for this resource** (audience binding per RFC 8707). Token is invalid if its `aud` or `resource` claim does not match `https://mcp.grooveos.app/mcp`.
...
```json
{
  "issuer": "https://api.grooveos.app",
  "authorization_endpoint": "https://api.grooveos.app/oauth/authorize",
  "token_endpoint": "https://api.grooveos.app/oauth/token",
  "registration_endpoint": "https://api.grooveos.app/oauth/register",
  "revocation_endpoint": "https://api.grooveos.app/oauth/revoke",
  "introspection_endpoint": "https://api.grooveos.app/oauth/introspect",
...
**`GET /oauth/authorize`** — Standard OAuth authorize redirect with PKCE `code_challenge` + `code_challenge_method=S256` + `resource=https://mcp.grooveos.app/mcp`. Render a consent screen, authenticate user via GitHub, collect team selection.
...
**`POST /oauth/introspect`** — Used by mcp-brain's `IntrospectionTokenVerifier` to validate tokens. Returns `{ active: true, sub: "<user_id>", team_scope: "<slug>", scope: "brain:read brain:write", aud: "https://mcp.grooveos.app/mcp" }`.
...
- **RS** (mcp-brain): runs `FastMCP(token_verifier=IntrospectionTokenVerifier(introspection_endpoint="https://api.grooveos.app/oauth/introspect"), auth=AuthSettings(issuer_url="https://api.grooveos.app", resource_server_url="https://mcp.grooveos.app/mcp"))`. The RS only serves `/.well-known/oauth-protected-resource` and validates tokens — no user-facing login.
...
User clicks "Add connector" in Claude.ai
  → Claude.ai → GET /oauth/authorize?...&resource=https://mcp.grooveos.app/mcp&code_challenge=...
  ...
  → memory-api verifies PKCE, issues OAuth access token:
      { access_token: "oat_...", team_scope: "<slug>", sub: "<user_id>", aud: "https://mcp.grooveos.app/mcp" }
...
**Fix:** Verify the path. If user pastes `https://mcp.grooveos.app/mcp`, FastMCP's default `/mcp` is correct. If they paste `https://mcp.grooveos.app/`, set `streamable_http_path="/"`. Stick with `/mcp` — it's already what mcp-brain uses (current nginx proxies `/` → mcp-brain, FastMCP default path is `/mcp`, so user should paste `https://mcp.grooveos.app/mcp`).
[CITED: note.com/brave_quince241 — bug #2]

### Pitfall 2: Trailing-slash inconsistency in `resource` parameter
**What:** Claude.ai sends `resource=https://mcp.grooveos.app/mcp` (no trailing slash). If memory-api stores the audience as `https://mcp.grooveos.app/mcp/`, token audience validation fails with a silent 401.
...
**What:** mcp.grooveos.app nginx currently proxies all requests to mcp-brain. The `/.well-known/oauth-protected-resource` path must reach mcp-brain (good — it's already proxied). The `/.well-known/oauth-authorization-server` path must reach **memory-api at api.grooveos.app** — it's a different vhost, so Claude.ai will fetch it at `https://api.grooveos.app/.well-known/oauth-authorization-server`. Current api.grooveos.app nginx (20-api.conf) only proxies `/v1/` — it does **not** proxy `/.well-known/`. This is a required nginx change.
...
**Fix:** Confirm `/.well-known/` requests to mcp.grooveos.app are covered by the existing CORS block. For api.grooveos.app, a new `location /.well-known/` block must include CORS headers.
...
**What:** Claude.ai includes `resource=https://mcp.grooveos.app/mcp` in the token request. The token endpoint must accept and record this. The issued token's `aud` must match. mcp-brain's introspection-based validator must check this (when `--oauth-strict` mode is set in the SDK reference example).
...
| `infrastructure/nginx/conf.d/20-api.conf` | Modified | nginx | Add `location /.well-known/` block for api.grooveos.app → memory-api:8000 |
| `infrastructure/nginx/conf.d/20-api.conf` | Modified | nginx | Add `location /oauth/` block for api.grooveos.app → memory-api:8000 |
...
| A4 | CORS on `/.well-known/` at api.grooveos.app is currently absent and must be added | §5, Pitfall 6 | If CORS is already present on `*` at that vhost, no change needed |
