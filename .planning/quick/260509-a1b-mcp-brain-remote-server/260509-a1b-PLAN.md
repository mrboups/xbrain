...
must_haves:
  truths:
    - "POST /mcp on mcp.grooveos.app routes to FastMCP sidecar (mcp-brain:8104)"
    ...
    - "nginx 40-mcp.conf serves mcp.grooveos.app with CORS headers"
...
ChatGPT: OpenAPI 3.0 spec for Custom GPT Actions (points to api.grooveos.app directly).
...
New server block for mcp.grooveos.app routing to mcp-brain:8104. Include CORS headers required by browser MCP clients (Claude.ai web + ChatGPT web).

```nginx
# === mcp-brain remote MCP server at mcp.grooveos.app ===
server {
  listen 80;
...
This spec describes the key memory-api endpoints directly (ChatGPT calls api.grooveos.app, not the MCP server). Covers the 4 most useful tools for a GPT: memory search/add + task create/list.
...
```json
{
  ...
  "servers": [{"url": "https://api.grooveos.app"}],
  "security": [{"bearerAuth": []}],
  "components": {
    "securitySchemes": {
      "bearerAuth": {
...
### Verify
- `infrastructure/nginx/conf.d/40-mcp.conf` exists with mcp.grooveos.app server block
