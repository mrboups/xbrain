...
**Task Goal:** Build mcp-brain remote MCP server for Claude.ai + ChatGPT web access to team brain — new FastMCP sidecar (port 8104) with 9 tools, personal API token endpoint in memory-api, nginx vhost mcp.example.com, docker-compose entry, OpenAPI spec for ChatGPT Actions.
**Verified:** 2026-05-09
**Status:** PASSED
...
|---|-------|--------|----------|
| 1 | POST /mcp on mcp.example.com routes to FastMCP sidecar (mcp-brain:8104) | COVERED | `40-mcp.conf` proxies all requests to `http://mcp-brain:8104`; docker-compose exposes port 8104 on `xbrain-mcp-brain` |
...
| 8 | nginx 40-mcp.conf serves mcp.example.com with CORS headers | COVERED | `40-mcp.conf` has OPTIONS preflight returning 204, `Access-Control-Allow-Origin: *`, `Access-Control-Allow-Headers` including `Mcp-Session-Id`, `proxy_buffering off` for SSE |
| 9 | OpenAPI spec chatgpt-actions.json in apps/mcp-brain/ | COVERED | `apps/mcp-brain/chatgpt-actions.json` is valid OpenAPI 3.0 JSON with `servers: [{url: "https://api.example.com"}]`, bearerAuth scheme, 5 paths (memory/search, memory/upsert, tasks, crm/contacts, me) |
...
| `infrastructure/nginx/conf.d/40-mcp.conf` | VERIFIED | 37 lines, mcp.example.com server block, CORS preflight, proxy to mcp-brain:8104, SSE support |
...
| `apps/mcp-brain/chatgpt-actions.json` | VERIFIED | OpenAPI 3.0.0 with servers=[api.example.com], bearerAuth, 5 paths covering memory/tasks/contacts/crm/me |
...
1. **DNS for mcp.example.com**
   - Test: `curl -I https://mcp.example.com/nginx-health` from external
...
2. **MCP protocol handshake with Claude.ai**
   - Test: Add `https://mcp.example.com` as a remote MCP server in Claude.ai settings with a valid xbt_ token
