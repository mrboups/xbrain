  ...
  async (accessToken, refreshToken, idToken, profile, cb) => {
    try {
      ...
      if (existingUser?.provider === provider) {
        ...
        // so that both GitHub and Google (or any other provider) work for the same account.
        if (id && !existingUser[providerKey]) {
          await updateUser(existingUser._id, { [providerKey]: id });
          existingUser[providerKey] = id;
          logger.info(
            `[${provider}Login] Linked ${provider} to existing user ${email} (was: ${existingUser.provider})`,
          );
        }
        await handleExistingUser(existingUser, avatarUrl, appConfig, email);
        return cb(null, existingUser);
