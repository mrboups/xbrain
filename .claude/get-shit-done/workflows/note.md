...
- **Global scope**: `D:/VSC/xbrain/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/VSC/xbrain/.claude/notes/`)
...
2. Glob `D:/VSC/xbrain/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/VSC/xbrain/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/VSC/xbrain/.claude/notes/` — works in any directory
